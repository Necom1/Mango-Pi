    ...
    def __init__(self, bot: commands.Bot, pack: dict):
        ...
        for i in range(len(pack["custom"])):
            ...
            if pack["custom"][i]:
                ...
                self.emote_id_to_emote.update({pack["emote"][i]: emote})
            if role and emote:
...
class RoleMenu(commands.Cog):
    ...
    async def set(self, ctx: commands.Context, name: str, mes: int, chan: discord.TextChannel = None):
        ...
        if chan.id == find.channel and mes.id == find.message_id:
            return await ctx.send(f"Received same input as one stored in database, no changes made.")
        self.db.update_one({"guild_id": ctx.guild.id, "name": name}, {"$set": {
